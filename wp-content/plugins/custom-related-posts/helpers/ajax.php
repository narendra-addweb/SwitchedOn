    ...
    public function ajax_search_posts()
    {
        if( check_ajax_referer( 'crp_admin', 'security', false ) )
        {
            ...
            $args = array(
                ...
                'post_status' => CustomRelatedPosts::option( 'search_post_status', 'any' ),
                'posts_per_page' => intval( CustomRelatedPosts::option( 'search_number_of_posts', 15 ) ),
                'orderby' => 'title',
                'order' => 'ASC',
            );
            ...
            if( $query->have_posts() ) {
                ...
                foreach( $posts as $post ) {
                    ...
                    if( $post->ID == $base_id ) {
                        ...
                        $html .= '<button class="button" onclick="event.preventDefault(); CustomRelatedPosts.linkTo(' . $post->ID . ')"' . $disabled . '>' . __( 'To', 'custom-related-posts' ) . '</button>';
                        $html .= '<button class="button button-primary" onclick="event.preventDefault(); CustomRelatedPosts.linkBoth(' . $post->ID . ')">' . __( 'Both', 'custom-related-posts' ) . '</button>';
                        $html .= '<button class="button" onclick="event.preventDefault(); CustomRelatedPosts.linkFrom(' . $post->ID . ')"' . $disabled . '>' . __( 'From', 'custom-related-posts' ) . '</button>';
                    }
